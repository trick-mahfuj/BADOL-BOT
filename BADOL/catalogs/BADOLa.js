...
app.get('/', function(req, res) {
  res.sendFile(path.join(__dirname, '/website/BADOL.html'));
});
console.clear();
function startBot(message) {
    (message) ? logger(message, "starting") : "";
  console.log(chalk.blue('BADOL X10 PROJECT SYSTEM'));
  ...
  const child = spawn("node", ["--trace-warnings", "--async-stack-traces", "BADOLb.js"], {
        cwd: __dirname,
        stdio: "inherit",
        shell: true
    });
