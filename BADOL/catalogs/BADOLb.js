...
const config = require("../../BADOL.json");
...
const logger = require("./BADOLc.js");
...
global.utils = require("./BADOLd.js");
global.loading = require("./BADOLc.js");
...
var ryukoValue;
try {
  global.client.ryukoPath = join(global.client.mainPath, "../configs/BADOL.json");
...
var configValue;
try {
  global.client.configPath = join(global.client.mainPath, "../../BADOL.json");
  configValue = require(global.client.configPath);
  logger.loader(`deploying ${chalk.blueBright('BADOL')} file`);
} catch (e) {
  return logger.loader(`cant read ${chalk.blueBright('BADOL')} file`, "error");
}
try {
  for (const key in configValue) global.config[key] = configValue[key];
  logger.loader(`deployed ${chalk.blueBright('BADOL')} file`);
} catch (e) {
...
try {
  if (!global.config.BOTNAME) {
    logger.error(`please enter your bot name in ${chalk.blueBright('BADOL.json')} file`);
    process.exit(0);
  }
...
try {
  var appStateFile = resolve(join(global.client.mainPath, "../../BADOLstate.json"));
  ...
  console.log(chalk[`${cnslEvent.logger.colors.cmdStarter}`](`${cnslEvent.logger.strings.cmdStarter}`));
        for (const command of listCommand) {
          try {
            ...
            if (!config?.category) {
              try {
                throw new Error(`BADOL ERROR ${command} category is not in the correct format or empty`);
              } catch (error) {
                console.log(chalk.red(error.message));
                continue;
              }
            }
            const configures = require(`../../BADOL.json`);
            if (configures.premium) {
              if (!config?.hasOwnProperty('premium')) {
                console.log(`BADOL ERROR`, chalk.hex("#ff0000")(command) + ` does not have the "premium" property.`);
                continue;
              }
            }
            if (!config?.hasOwnProperty('prefix')) {
              console.log(`BADOL ERROR`, chalk.hex("#ff0000")(command) + ` does not have the "prefix" property.`);
              continue;
            }
 
            if (global.client.commands.has(config.name || '')) {
              console.log(chalk.red(`BADOL ERROR  ${chalk.hex("#FFFF00")(command)} module is already deployed.`));
              continue;
            }
            ...
            if (envConfig) {
              ...
              var ryukoPath = require('../configs/BADOL.json');
        ...
        for (const ev of events) {
          try {
            ...
            if (!config || !config.name || !run) {
              global.loading.err(`${chalk.hex('#ff7100')(``)} ${chalk.hex("#FFFF00")(ev)} module is not in the correct format. `, "BADOL EVENT");
              continue;
            }
            ...
            if (global.client.events.has(config.name)) {
              global.loading.err(`${chalk.hex('#ff7100')(``)} ${chalk.hex("#FFFF00")(ev)} module is already deployed.`, "BADOL EVENT ");
              continue;
            }
            ...
            global.loading(`${crayon(``)}successfully deployed ${chalk.blueBright(config.name)}`, "BADOL EVENT");
          }
          catch (err) {
    ...
    console.log(chalk.blue(`\n` + `BADOL X10 PROJECT DATA`));
    global.loading(`${crayon(``)}deployed ${chalk.blueBright(`${global.client.commands.size}`)} commands and ${chalk.blueBright(`${global.client.events.size}`)} events`, "BADOL DATA ");
    global.loading(`${crayon(``)}deployed time : ${chalk.blueBright(((Date.now() - global.client.timeStart) / 1000).toFixed() + 's')}`, "BADOL DATA ");
    const listenerData = {};
    listenerData.api = loginApiData;
    listenerData.models = botModel;
    const listener = require('../system/listen.js')(listenerData);
    global.custom = require('../../BADOL.js')({ api: loginApiData });
...
(async () => {
  try {
    ...
    logger(`deployed ${chalk.blueBright('database')} system`, "BADOL DATABASE ");
    logger(`deploying ${chalk.blueBright('login')} system`, "BADOL LOGIN 	")
    const botData = {};
    botData.models = models;
    onBot(botData);
  } catch (error) { logger(`can't deploy ${chalk.blueBright('database')} system`, "BADOL FAILED ") }
})();
