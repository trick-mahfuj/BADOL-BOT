...
module.exports.onLoad = function () {
  ...
  const path = join(__dirname, "BADOL", "joinvideo");
  if (existsSync(path)) mkdirSync(path, { recursive: true }); 

  const path2 = join(__dirname, "BADOL", "joinvideo", "randomgif");
...
\n\n⋆✦⋆⎯⎯⎯⎯⎯⎯⎯⎯⎯⋆✦⋆`, attachment: fs.createReadStream(__dirname + "/BADOL/king.mp4")} ,threadID));
      ...
      try {
          ...
          const randomPath = readdirSync(join(__dirname, "BADOL", "joinGif", "randomgif"));

          if (existsSync(pathGif)) formPush = { body: msg, attachment: createReadStream(pathvideo), mentions }
          else if (randomPath.length != 0) {
              const pathRandom = join(__dirname, "BADOL", "joinGif", "randomgif", `${randomPath[Math.floor(Math.random() * randomPath.length)]}`);
